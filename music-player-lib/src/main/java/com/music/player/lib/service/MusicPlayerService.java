...
public class MusicPlayerService extends Service implements MusicPlayerPresenter, MediaPlayer.OnPreparedListener,
    ...
    //用户设定的内部播放器播放模式，默认MusicPlayModel.MUSIC_MODEL_LOOP,即列表循环播放
    ...
    public boolean isPlaying() {
        try {
            return mMediaPlayer != null &&
                    (mMusicPlayerState == MusicConstants.MUSIC_PLAYER_PREPARE // 处于 准备中
                            || mMusicPlayerState == MusicConstants.MUSIC_PLAYER_PLAYING // 处于 播放中
                            || mMusicPlayerState == MusicConstants.MUSIC_PLAYER_BUFFER); // 处于 缓冲中
        } catch (RuntimeException e) {
